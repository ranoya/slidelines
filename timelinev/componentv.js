...
let timelinev = function (id, arr, ano, titulo, conteudo) {
  ...
  html = `<div style='row-gap: 0 !important; display: grid !important; grid-template-columns: 200px 100px 1fr;' class='slidelinelvh'>`;
  htmlfinal = "";
  ...
  for (let k = 0; k < arr.length; k++) {
    ...
    if (anoatual != qualano) {
      html += `<div class="slidelinelvh_track1 ${ultimo}" style="grid-column: 1; grid-row: span ${quantosblocos}"><div class="slidelinelvh_track1_cont" style="display: inline-block; position: sticky; top: 0;">${arr[k][ano]}</div></div>`;
    }
    ...
    if (tituloatual != qualtitulo) {
      html += `<div class="slidelinelvh_track2 ${ultimo}" style="grid-column: 2; grid-row: span ${quantostitulos}"><div class="slidelinelvh_track2_cont" style="display: inline-block; position: sticky; top: 0;">${arr[k][titulo]}</div></div>`;
    }
    ...
    html += `<div class="slidelinelvh_track3 ${ultimo}" style="grid-column: 3;"><div class="slidelinelvh_track3_cont" style="display: inline-block; position: sticky; top: 0;">${arr[k][conteudo]}</div></div>`;
  }

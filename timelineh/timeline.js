...
fetch(arquivojson).then(response => response.json()).then((dados) => {
    ...
    while (i < dados.length) {
        ...
        anoscode += `<div class="linhadotempo" onclick='nowgo(${i})' style='display: inline-block; margin: 0; padding: 0; top: 0; left: 0; width: calc(5vw - 1px); margin-right: ${(contat - 1) * 5}vw; z-index: ${600 + i};'><div class='registroano' style='${milestone}'>${tituloatual}</div></div>`;
    ...
    document.getElementById("tempo").innerHTML = `<div id="fulltempo" style="width: calc(${(i + 19)*5}vw - var(--back-control-width));">${anoscode}</div>`;

...
fetch(arquivojson).then(response => response.json()).then((dados) => {
    ...
    while (i < dados.length) {
        ...
        tituloscode += `<div class="track" style='position: sticky; height: 60px; top: 0; left: ${i * 100}vw; z-index: 200; width: ${contat * 100}vw; background-color: ${dados[i].titulofundo}; color: ${dados[i].titulofrente}'>${tituloatual}</div>`;
